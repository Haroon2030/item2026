"""أصناف لها كمية في المخزن بلا سعر بيع على وحدة البيع.

غير مسعّر = رصيد AVL_QTY > 0 على الوحدة الرئيسية
            ولا يوجد صف سعر > 0 لوحدة البيع في IAS_ITEM_PRICE
            لنفس المخزن والمستوى 1.
"""
...
def _base_sql(schema: str, *, group_sql: str, item_sql: str) -> str:
    """رصيد على الوحدة الرئيسية · التسعير/العرض بوحدة البيع فقط."""
    return f"""
        SELECT /*+ LEADING(w dm ds m) USE_NL(dm ds m g) */
          m.I_CODE,
          m.I_NAME,
          ds.ITM_UNT,
          m.G_CODE,
          ...
          :lev AS LEV_NO,
          ROUND(
            w.AVL_QTY * NVL(dm.P_SIZE, 1) / NULLIF(NVL(ds.P_SIZE, 1), 0)
          , 3) AS AVL_QTY,
          ROUND(
            (
              CASE
              ...
              END
            ) * NVL(ds.P_SIZE, 1) / NULLIF(NVL(dm.P_SIZE, 1), 0)
          , 4) AS AVG_COST
        FROM {schema}.IAS_ITM_WCODE w
        JOIN {schema}.IAS_ITM_DTL dm
          ON dm.I_CODE = w.I_CODE
         AND dm.ITM_UNT = w.ITM_UNT
         AND NVL(dm.MAIN_UNIT, 0) = 1
        JOIN {schema}.IAS_ITM_DTL ds
          ON ds.I_CODE = w.I_CODE
         AND NVL(ds.SALE_UNIT, 0) = 1
         AND NVL(ds.P_SIZE, 0) > 0
        JOIN {schema}.IAS_ITM_MST m
          ON m.I_CODE = w.I_CODE
        ...
        WHERE w.W_CODE = :wh
          ...
          AND NOT EXISTS (
            ...
            WHERE p.LEV_NO = :lev
              AND p.I_CODE = w.I_CODE
              AND p.ITM_UNT = ds.ITM_UNT
              AND p.W_CODE = w.W_CODE
              AND p.I_PRICE > 0
          )
    """
...
def fetch_unpriced_items(
    *,
    ...
    """أصناف بلا سعر على وحدة البيع لمخزن ومستوى."""
    ...
    schema = _schema()
    params: dict[str, Any] = {"wh": _bind_wh(wh), "lev": lev}
...
def build_unpriced_excel(
    *,
    ...
    buf.write(
        ...
        f"<table><caption>رصيد بلا تسعير — مخزن {wh_label}"
        f" · وحدة البيع · مستوى السعر 1"
        ...
        "<th>غير مسعر</th><th>متوسط التكلفة</th>"
        "</tr></thead><tbody>"
    )
    for i, r in enumerate(report.get("rows") or [], start=1):
        ...
        buf.write('<td class="txt">غير مسعر</td>')
